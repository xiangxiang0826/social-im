...
func (l *StopMicLogic) StopMic(in *pb.StopMicReq) (*pb.StopMicResp, error) { //把mistatus 从1 置为 0
	...
	if (userMic != nil && userMic.UidType != 1) || (errs.RecordNotFound(err)) { //用户不是上麦的状态  或者没有麦位状态
		return nil, xerr.NewErrMsg("麦位状态有误， 无法进行停麦操作")
	}

	if userMic != nil && userMic.UidType == 1 && userMic.RoomId == in.Room {
		err = l.rep.RoomManagerOnMicer.UpdateMicStatus(l.ctx, l.rep.Mysql, in.User, in.Room, 0)
		...
		_, err = l.svcCtx.UserRpc.SendRtm(l.ctx, &pb.SendRtmReq{From: "1", To: suser, MessageType: agora.STOPMIC, MessageBody: string(msgdata)}) //给房主发消息
		_, err = l.svcCtx.UserRpc.SendRtm(l.ctx, &pb.SendRtmReq{From: "1", To: roomCreater, MessageType: agora.STOPMIC, MessageBody: string(msgdata)})
		...
		return &pb.StopMicResp{Status: &res}, err
