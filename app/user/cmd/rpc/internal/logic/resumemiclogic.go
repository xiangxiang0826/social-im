...
func (l *ResumeMicLogic) ResumeMic(in *pb.ResumeMicReq) (*pb.ResumeMicResq, error) { //把mistatus 从0 置为 1
	...
	if (userMic != nil && userMic.UidType != 0) || (errs.RecordNotFound(err)) { //用户不是停麦的状态  或者没有麦位状态
		return nil, xerr.NewErrMsg("麦位状态有误， 无法进行恢复麦位操作")
	}

	if userMic != nil && userMic.UidType == 0 && userMic.RoomId == in.Room {

		err = l.rep.RoomManagerOnMicer.UpdateMicStatus(l.ctx, l.rep.Mysql, in.User, in.Room, 1)
		if err != nil {
			return nil, err
		}
		...
		_, err = l.svcCtx.UserRpc.SendRtm(l.ctx, &pb.SendRtmReq{From: "1", To: ssuser, MessageType: agora.RESUMEMIC, MessageBody: string(msgdata)}) //给房主发消息
		_, err = l.svcCtx.UserRpc.SendRtm(l.ctx, &pb.SendRtmReq{From: "1", To: roomCreater, MessageType: agora.RESUMEMIC, MessageBody: string(msgdata)})
		...
		return &pb.ResumeMicResq{Status: &res}, err
	}
