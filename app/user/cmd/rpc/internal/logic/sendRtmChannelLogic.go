...
type SendRtmChannelLogic struct {
	ctx    context.Context
...
func NewSendRtmChannelLogic(ctx context.Context, svcCtx *svc.ServiceContext) *SendRtmChannelLogic {
	return &SendRtmChannelLogic{
		ctx:         ctx,
		svcCtx:      svcCtx,
...
func (l *SendRtmChannelLogic) SendRtmChannel(in *pb.SendRtmChannelReq) (*pb.SendRtmChannelResp, error) {
	...
	urlstr := fmt.Sprintf("https://%s/dev/v2/project/%s/rtm/users/%s/channel_messages", host, appID, in.From)
	...
	return &pb.SendRtmChannelResp{}, nil
}
