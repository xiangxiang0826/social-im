package user

import (
	"context"
	"social-im/app/user/cmd/rpc/pb"

	"social-im/app/user/cmd/api/internal/svc"
	"social-im/app/user/cmd/api/internal/types"

	"github.com/zeromicro/go-zero/core/logx"
)

type RejectMicLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewRejectMicLogic(ctx context.Context, svcCtx *svc.ServiceContext) *RejectMicLogic {
	return &RejectMicLogic{
		Logger: logx.WithContext(ctx),
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

func (l *RejectMicLogic) RejectMic(req *types.RejectMicReq) (resp *types.RejectMicResp, err error) {
	// todo: add your logic here and delete this line
	var mresp types.RejectMicResp
	rep, err := l.svcCtx.UserRpc.RejectMic(l.ctx, &pb.RejectMicReq{Room: req.Room, Uid: req.Uid, User: req.User})

	if rep != nil {
		mresp.Status.Status = rep.Status.Status
		mresp.Status.Code = int(rep.Status.Code)
		return &mresp, nil

	}
	return nil, err
	return
}
